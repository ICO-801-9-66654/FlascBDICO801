...
def maestros_nuevo():
    ...
    if request.method == 'POST' and create_form.validate():
        matricula_ingresada = create_form.matricula.data
        maestro_existente = Maestros.query.filter_by(matricula=matricula_ingresada).first()
        if maestro_existente:
            flash(f"No es posible registrar: La matrícula {matricula_ingresada} ya existe.", "error")
        else:
            maestro = Maestros(
                matricula=matricula_ingresada,
